...
int main (int argc, char** argv) {
  ...
  hsize_t block[2] = {NX/(dim[0]*2), NY/(dim[1]*2)}; // 1つのブロックのサイズは全体の1/4
  hsize_t offset[2] = {(mpirank / dim[0]) * block[0], (mpirank % dim[0]) * block[1]};  // ブロックの開始地点
  hsize_t count[2] = {2, 2}; // 各プロセスが配置するブロック数は2x2
  hsize_t stride[2] = {NX/dim[0], NY/dim[1]}; // ブロック間の間隔
  ...
  H5Sselect_hyperslab(globalspace, H5S_SELECT_SET, offset, stride, count, block); // Nlocal -> block
  H5Pclose(plist);
